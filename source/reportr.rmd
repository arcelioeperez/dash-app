...
testset$charges <- ifelse(testset$charges <= 10000, 0, 1)

confusionMatrix(factor(prediction, levels = min(testset$charges):max(testset$charges)), 
      factor(testset$charges, levels = min(testset$charges):max(testset$charges)))
